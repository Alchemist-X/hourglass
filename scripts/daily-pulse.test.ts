...
describe("daily pulse script", () => {
  ...
  it("sets pizza as the default env plus live pulse-direct defaults", () => {
    ...
    const previousExecutionMode = process.env.AUTOPOLY_EXECUTION_MODE;
    const previousStrategy = process.env.AGENT_DECISION_STRATEGY;
    delete process.env.ENV_FILE;
    delete process.env.AUTOPOLY_EXECUTION_MODE;
    ...
    try {
      ...
      expect(command.env.AUTOPOLY_EXECUTION_MODE).toBe("live");
      ...
      if (previousExecutionMode === undefined) {
        ...
        process.env.AUTOPOLY_EXECUTION_MODE = previousExecutionMode;
      }
