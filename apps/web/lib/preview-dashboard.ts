import type { OverviewResponse, PublicPosition, PublicTrade } from "@autopoly/contracts";
...
function polymarketEventUrl(eventSlug: string): string {
  return `https://polymarket.com/event/${eventSlug}`;
}

export function buildPolymarketMarketUrl(eventSlug: string, marketSlug?: string | null): string {
...
async function fetchEventClusters(
  ...
  for (const position of positions) {
    uniqueKeys.set(position.event_slug, { marketSlug: position.market_slug });
  }

  for (const example of pulseExamples) {
    uniqueKeys.set(example.event_slug, { marketSlug: example.market_slug });
  }
...
function buildTrackedNavSeries(
  ...
  const currentPriceByToken = new Map(positions.map((position) => [position.token_id, position.current_price]));
...
function buildPositionInsights(
  ...
  return positions.slice(0, 6).map((position) => {
    const pulse = pulseExamples.find((item) => item.market_slug === position.market_slug || item.event_slug === position.event_slug);
    const cluster = clusterByEventSlug.get(position.event_slug) ?? { slug: "other", label: MARKET_CLUSTER_META.other.label };
    ...
    return {
      ...
      market_url: buildPolymarketMarketUrl(position.event_slug, position.market_slug)
    };
  });
}
...
function buildClusters(
  ...
  for (const position of positions) {
    ...
    if (!existing.event_slugs.includes(position.event_slug)) {
      existing.event_slugs.push(position.event_slug);
    }
...
function buildMarketClusters(positionInsights: PreviewPositionInsight[]): PreviewMarketCluster[] {
  ...
  for (const insight of positionInsights) {
    ...
    existing.items.push({
      id: insight.id,
      market_slug: insight.market_slug,
      event_slug: insight.event_slug,
      label: formatMarketLabel(insight.market_slug),
      url: insight.market_url,
      value_usd: round(insight.current_value_usd, 2),
      last_seen_at_utc: null,
      source_tags: [insight.cluster_label, insight.outcome_label]
    });
...
function buildAgentFeed(
  ...
  for (const example of pulseExamples.slice(0, 3)) {
    for (const source of (example.sources ?? []).slice(0, 3)) {
      entries.push({
        ...
        market_slug: example.market_slug,
        market_url: buildPolymarketMarketUrl(example.event_slug, example.market_slug),
        tone: "neutral"
      });
    }
    ...
    entries.push({
      ...
      phase: "score",
      label: "SCORE",
      detail: `${example.market_slug} · 推荐 ${example.recommended_notional_usd.toFixed(2)} USD`,
      market_slug: example.market_slug,
      market_url: buildPolymarketMarketUrl(example.event_slug, example.market_slug),
      tone: "neutral"
    });
    ...
    for (const trade of example.executed_trades) {
      entries.push({
        id: `${example.run_id}-${trade.order_id ?? trade.market_slug}`,
        timestamp_utc: trade.timestamp_utc ?? example.generated_at_utc,
        phase: "execution",
        label: "EXECUTE",
        detail: `${trade.side} ${trade.market_slug} · ${trade.filled_notional_usd.toFixed(2)} USD`,
        market_slug: trade.market_slug,
        market_url: buildPolymarketMarketUrl(trade.event_slug, trade.market_slug),
        tone: "positive"
      });
    }
  }
  ...
  if (entries.length <= 1) {
    for (const trade of recentTrades.slice(0, 6)) {
      entries.push({
        ...
        phase: "execution",
        label: "EXECUTE",
        detail: `${trade.side} ${trade.market_slug} · ${(trade.filled_notional_usd || trade.requested_notional_usd).toFixed(2)} USD`,
        market_slug: trade.market_slug,
        market_url: buildPolymarketMarketUrl(trade.market_slug, trade.market_slug),
        tone: "positive"
      });
    }
  }
  ...
  for (const activity of recentActivity.slice(0, 3)) {
    entries.push({
      ...
      phase: "search",
      label: activity.type,
      detail: `${activity.side ?? activity.direction} · ${formatMarketLabel(activity.market_slug)}`,
      market_slug: activity.market_slug,
      market_url: buildPolymarketMarketUrl(activity.event_slug, activity.market_slug),
      tone: "neutral"
    });
  }
