...
import { buildPolymarketMarketUrl, formatMarketLabel } from "../lib/preview-dashboard";
...
function phaseInstruction(entry: PreviewAgentFeedEntry): string {
  const target = formatMarketLabel(entry.market_slug ?? "portfolio");
...
function LatestPulseCard({ data }: { data: PreviewDashboardData }) {
  ...
  return (
    <article className="bal-side-card">
      ...
      <a className="bal-inline-link" href={example.market_url ?? `https://polymarket.com/event/${example.event_slug}`} target="_blank" rel="noreferrer">
        打开对应市场
      </a>
    </article>
  );
      ...
      <article className="bal-side-panel bal-camp-panel">
        ...
        <div className="bal-camp-log">
          ...
          <article className="bal-camp-entry">
            ...
            <p>{pulseSummary}</p>
            {data.latestPulse ? (
              <a className="bal-inline-link" href={data.latestPulse.market_url ?? `https://polymarket.com/event/${data.latestPulse.event_slug}`} target="_blank" rel="noreferrer">
                打开对应市场
              </a>
            ) : null}
          </article>
          ...
          <article className="bal-camp-entry">
            ...
            {recentTrade ? (
              <>
                ...
                <p>{formatMarketLabel(recentTrade.market_slug)} 已进入公开围观记录，用户可以继续点进 Polymarket 看真实市场。</p>
                <a className="bal-inline-link" href={buildPolymarketMarketUrl(recentTrade.market_slug, recentTrade.market_slug)} target="_blank" rel="noreferrer">
                  去 Polymarket
                </a>
              </>
            ) : (
        ...
        <div className="bal-terminal-current">
          ...
          <div className="bal-terminal-current-meta">
            ...
            {currentEntry.market_url ? (
              <a href={currentEntry.market_url} target="_blank" rel="noreferrer">打开 Polymarket 市场</a>
            ) : null}
          </div>
        </div>
      ) : null}
            ...
            <article key={`${entry.id}-${index}`} className="bal-terminal-line">
              ...
              <div className="bal-terminal-entry">
                ...
                <div className="bal-terminal-queue-meta">
                  ...
                  <span>{entry.market_slug ? formatMarketLabel(entry.market_slug) : "portfolio"}</span>
                </div>
              </div>
            </article>
          ))}
        ...
        <article key={cluster.key} className="bal-cluster-card">
          ...
          <ul className="bal-cluster-links">
            {cluster.items.slice(0, 2).map((item) => (
              <li key={item.id}>
                <a href={item.url} target="_blank" rel="noreferrer">{formatMarketLabel(item.market_slug)}</a>
              </li>
            ))}
          </ul>
        </article>
        ...
        <article key={item.id} className="bal-position-card">
          <div className="bal-position-head">
            <div>
              ...
              <h3>{formatMarketLabel(item.market_slug)}</h3>
            </div>
            <a href={item.market_url} target="_blank" rel="noreferrer">Polymarket</a>
          </div>
          <div className="bal-position-metrics">
            ...
            <div>
              <span>持仓逻辑</span>
              <strong>{item.outcome_label}</strong>
            </div>
          </div>
  ...
  return (
    <div className="bal-preview-root bal-preview-core">
      <NavHero
        ...
        note="这版更像克制的基金首页，先给净值波动，再给搜索、判断和执行过程。"
        aside={
      ...
      <section className="bal-bento-grid">
        <article className="bal-panel is-wide">
          <div className="bal-panel-head">
            <p className="bal-eyebrow">Event Clusters</p>
  ...
  return (
    <div className="bal-preview-root bal-preview-flow">
      <NavHero
        ...
        note="这是一个可真实运行、可公开围观、具备服务层硬风控的 Polymarket 自主交易系统。网页先展示单位净值，再慢慢回放 Agent 如何搜索、判断和调仓。"
